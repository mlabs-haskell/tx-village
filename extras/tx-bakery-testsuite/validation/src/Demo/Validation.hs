...
import LambdaBuffers.Demo.Plutus.Plutarch (
  EqDatum,
  EqRedeemer (EqRedeemer'IsEqual, EqRedeemer'IsNotEqual),
 )
...
{- | `eqValidator dat rdmr ctx` checks whether the Datum `dat` is (not)equal to
      the Datum supplied in Redeemer.
-}
eqValidator :: ClosedTerm (V3.PScriptContext :--> PUnit)
eqValidator = plam $ \ctx' -> ptraceInfo "[Mint]" $ unTermCont $ do
  ctx <- pmatchC ctx'

  V3.PSpendingScript _ mayDatum <- pmatchC $ V3.pscriptContext'scriptInfo ctx
